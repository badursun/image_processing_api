    ...
    try {
        ...
        res.send(
            ...
            .resize(
                ...
                h ?
                parseInt(h) :
                null, { fit, background: bg ? "#" + bg : null }
            )
            .withMetadata()
            .toBuffer()
        );
...
app.get("/all-images", async (req, res, next) => {
    const files = fs
        .readdirSync("./public")
        .map((file) => ({ url: `${req.protocol}://${req.get('host')}/public/${file}` }));
    const response =
        files.length > 0 ?
        files :
        {
            message: "No images found in public directory. Navigate to /upload to upload images images.",
